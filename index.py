...
CONFIG = {
    ...
    "min_confirmations_to_count": 3,

    "pallet_smooth_window": 6,
    "pallet_smooth_min_hits": 3,

    "match_x_threshold": 0.40,
    "match_y_threshold": 0.40,
    "match_area_ratio_threshold": 4.0,
    "match_aspect_ratio_threshold": 2.5,
...
class PalletSmoother:
    def __init__(self, window: int, min_hits: int):
        self.window = window
        self.min_hits = min_hits
        self._history: List[Optional[List[BBox]]] = []
        self._last_stable: List[BBox] = []

    def update(self, raw_pallets: List[BBox]) -> List[BBox]:
        self._history.append(raw_pallets if raw_pallets else None)
        if len(self._history) > self.window:
            self._history.pop(0)

        hits = sum(1 for h in self._history if h)
...
class GlobalFusionCounter:
    ...
    def _spatial_score(self, bbox: BBox, fw: int, fh: int, obj: GlobalObject) -> Optional[float]:
        ...
        if area_r   > self.match_area:   return None
        if aspect_r > self.match_aspect: return None

        return (abs(cx - ex) * 0.25
                + abs(cy - ey) * 0.75
                + (area_r - 1)
                + (aspect_r - 1) * 0.5)

    def _find_match(self, cam: str, lid: int,
                    bbox: BBox, fw: int, fh: int, now: float) -> Optional[int]:
        key = (cam, lid)
        ...
        for gid, obj in self.objects.items():
            if now - obj.last_seen > self.ttl * 2:
                continue
            score = self._spatial_score(bbox, fw, fh, obj)
        ...
        for obs in sorted(observations, key=lambda o: o["conf"], reverse=True):
            ...
            if gid is None:
                ...
                self.objects[gid] = GlobalObject(
                    ...
                    bbox=bbox, conf=conf, frame_w=fw, frame_h=fh,
                    last_seen=now,
                )

            obj = self.objects[gid]

            if cam not in obj.seen_cameras or obj.camera_name == cam:
                obj.observations += 1
            ...
            if not obj.counted and obj.observations >= self.min_confirmations:
                ...
                log.info(f"Box confirmed (global_id={gid}, "
                         f"cameras={obj.seen_cameras})")

            self._local_to_global[(cam, lid)] = gid

        stale = [gid for gid, obj in self.objects.items()
                 if now - obj.last_seen > self.ttl * 4]
        for gid in stale:
            del self.objects[gid]

        for key in [k for k, v in self._local_to_global.items()
...
def run_inference(
    ...
    device: str,
    cfg: dict,
) -> Tuple[List[Tuple[BBox, float]], List[BBox], List[Tuple[BBox, float]]]:
    ...
    return raw_boxes, pallet_boxes, []


def filter_countable(
    raw_boxes: List[Tuple[BBox, float]],
    stable_pallets: List[BBox],
) -> List[Tuple[BBox, float]]:
    if not stable_pallets:
        return []
    return [
        (bbox, conf)
        for bbox, conf in raw_boxes
        if any(boxes_overlap(bbox, p) for p in stable_pallets)
    ]


def draw_frame(
    frame: np.ndarray,
    box_tracks: List[LocalTrack],
    pallet_boxes: List[BBox],
    cam_label: str,
    cfg: dict,
) -> np.ndarray:
...
def draw_hud(
    ...
    cv2.putText(canvas, f"Input: {typing_buf or '_'} [digits+Enter | Backspace | r=reset | q=quit]",
        (900, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (180, 180, 180), 1)


def handle_arduino(
    arduino: Optional[ArduinoController],
    total: int,
    expected: Optional[int],
) -> None:
...
def main() -> None:
    ...
    pallet_smoothers = [
        PalletSmoother(
            window=CONFIG["pallet_smooth_window"],
            min_hits=CONFIG["pallet_smooth_min_hits"],
        )
        for _ in cameras
    ]
        ...
        while True:
            ...
            for i, cam in enumerate(cameras):
                ...
                if not ok or frame is None:
                    ...
                    cv2.putText(placeholder, f"Camera {i+1} offline",
                                (40, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
                    panels.append(placeholder)
                    pallet_smoothers[i].update([])
                    continue

                raw_boxes, raw_pallets, _ = run_inference(model, frame, device, CONFIG)

                stable_pallets = pallet_smoothers[i].update(raw_pallets)
                countable_boxes = filter_countable(raw_boxes, stable_pallets)
                tracks = trackers[i].update(raw_boxes, now)

                countable_bboxes = {bbox for bbox, _ in countable_boxes}
                observations.extend(
                    {
                    ...
                    for t in tracks if t.bbox in countable_bboxes
                )

                annotated = draw_frame(frame, tracks, stable_pallets, f"Cam {i+1}", CONFIG)

                cv2.rectangle(annotated, (0, 0), (380, 80), (0, 0, 0), -1)
                cv2.putText(annotated, f"Cam {i+1}  tracks: {len(tracks)}",
                            (12, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(annotated, f"Pallets: {len(stable_pallets)}",
                            (12, 62), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 120, 0), 2)
            ...
            if current_status != last_status:
                ...
                log.info(
                    f"Count={total}  Expected={expected_qty}  "
                    f"Status={'MATCH' if expected_qty is not None and total == expected_qty else 'NO MATCH' if expected_qty is not None else 'AWAITING'}"
                )
            ...
            draw_hud(canvas, fps, total, expected_qty, typing_buf,
                     arduino is not None and arduino.connected)
            cv2.imshow(CONFIG["window_name"], canvas)

            if cv2.getWindowProperty(CONFIG["window_name"], cv2.WND_PROP_VISIBLE) < 1:
                log.info("Window closed manually by user.")
                break
            ...
            while True:
                ...
                if key == 255:
                    ...
                    if typing_buf:
                        ...
                        try:
                            api_url = f"http://localhost:8000/Pallets/{barcode}"
                            response = requests.get(api_url, timeout=2.0)
                            
                            if response.status_code == 200:
                                data = response.json()
                                expected_qty = data["Quantity"]
                                
                                handle_arduino(arduino, False, expected_qty)
                                last_status = None
                                
                                log.info(f"SUCCESS: Barcode '{barcode}' loaded. Expected Qty: {expected_qty}")
                            elif response.status_code == 404:
                                log.warning(f"ERROR: Barcode '{barcode}' not found in database.")
                            else:
                                log.error(f"API returned status {response.status_code}")
                                
                        except requests.exceptions.RequestException as e:
                            log.error(f"API Connection Failed! Error: {e}")
